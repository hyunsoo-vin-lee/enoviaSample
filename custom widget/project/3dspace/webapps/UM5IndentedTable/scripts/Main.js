...
function executeWidgetCode() {
    require([
        ...
        var myWidget = {
            ...
            loadRootData: function(oids, doInit) {
                ...
                UM5ToolsWS.objInfo({
                    ...
                    onOk: function(data, callbackData) {
                        ...
                        //CUSTOM START
                        if ( doInit != null && doInit != undefined && doInit == true ) 
                        {
                        	for (var j = 0; j < myWidget.dataFull.length; j++) {
                                var testObj = myWidget.dataFull[j]; // 계속 row가 지워지므로 0만 바라본다.
                                myWidget.removeRootId(testObj.id);
                                j--;
                            }
                        }
                        //CUSTOM END
            ...
            //CUSTOM START
            addIds : function(paramJson) {
            	var ids = paramJson.ids;
            	myWidget.loadRootData( ids, paramJson.doInit );
            	for (var i in ids)
            	{
            		myWidget.oidsRoots.push(ids[i]);
            	}
            }
            //CUSTOM END
        };
