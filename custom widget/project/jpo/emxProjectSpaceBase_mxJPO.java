...
//    private final static String SELECT_ATTRIBUTE_TASK_ESTIMATED_END_DATE = "attribute[" + DomainConstants.ATTRIBUTE_TASK_ESTIMATED_FINISH_DATE  + "]";
    protected final static String SELECT_ATTRIBUTE_TASK_ESTIMATED_END_DATE = "attribute[" + DomainConstants.ATTRIBUTE_TASK_ESTIMATED_FINISH_DATE  + "]";
        ...
        try
        {
    		...
    		if( !(resultList.size() == 2 && "User Projects".equals(resultList.get(1)))){
    			...
    			while(itr.hasNext())
    				...
    				String oid = (String)folderMap.get(DomainConstants.SELECT_ID);
    ...
    public void modifyPrimaryOwnership(Context context, String[] args) throws Exception
    {
  	...
  	public void createAndConnectProject(Context context,String[]args)throws Exception
  	{
  		...
  		try {
			...
			String sTypeActual = (String) requestMap.get("TypeActual");
			
			if ( "TestProject".equals(sTypeActual) || "TestProductProject".equals(sTypeActual) )
			{
				String sTestProjectId = (String) paramMap.get("newObjectId");
				String sName = MqlUtil.mqlCommand(context, "print bus $1 select $2 dump", sTestProjectId, "name") + " Schedule";
				String sDate = (String) requestMap.get("ProjectDate");
				String sTimezone = (String) requestMap.get("timeZone");
				
				requestMap.put("TypeActual", "Project Space");
				requestMap.put("Name", sName);
				requestMap.put("ProjectDate", sDate);
				requestMap.put("timeZone", sTimezone);
				requestMap.remove("objectId");
				
				System.out.println("emxProjectSpaceBase : createAndConnectProject : requestMap - " + requestMap);
				Map commonProject = createNewProject(context, JPO.packArgs(requestMap));
				System.out.println("emxProjectSpaceBase : createAndConnectProject : commonProject - " + commonProject);
				String sCommonProjectId = (String) commonProject.get("id");
				
				MqlUtil.mqlCommand(context, "connect bus $1 relationship $2 to $3", sTestProjectId, "Subtask", sCommonProjectId);
			}
  	...
  	public MapList getActiveQuestionList(Context context,String[]args)throws Exception
  	{
		...
		for(int i=0; i < SelectedProjectList.size(); i++){
			...
			objectInfoList = DomainObject.getInfo(context, SelectedIds, objectSelects);
		}
		...
		for (Iterator iterator = objectInfoList.iterator(); iterator.hasNext();) {
			Map projectInfo = (Map) iterator.next();
			
			String sId = (String) projectInfo.get(DomainConstants.SELECT_ID);
			String sType = (String) projectInfo.get(DomainConstants.SELECT_TYPE);
			
			if(sType.equalsIgnoreCase(DomainConstants.TYPE_PROJECT_TEMPLATE)){
				selectedTemplateIdList.add(sId);
			}
		}
 ...
 public boolean isDPGInstalledandHasShadowGates(Context context,String args[]) throws Exception
 {
	...
	public static void copyPartialScheduleProcess(Context context,String[]args)throws Exception {
		...
		for (Iterator iterator = objectInfoList.iterator(); iterator.hasNext();) {
			...
			if(taskType.equalsIgnoreCase(TYPE_PROJECT_SPACE) || taskType.equalsIgnoreCase(TYPE_PROJECT_TEMPLATE)){
   ...
   public boolean isAddingShadowGateEnabled(Context context,String args[]) throws Exception
   {
    ...
    private boolean areDatesEqual(Date firstDate,Date secondDate){
	  ...
	  /**
	    ...
	    public MapList getProjectTemplatesAndProjects(Context context, String args[]) throws Exception {
	    	...
	    	objectSelectables.add(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT);
	    	objectSelectables.add(ProgramCentralConstants.SELECT_KINDOF_PROJECT_BASELINE);
	    	objectSelectables.add(SELECT_CURRENT);
	    	
	    	String whereClause = EMPTY_STRING;
	    	String type =  TYPE_PROJECT_SPACE +","+ TYPE_PROJECT_TEMPLATE;
			...
			while(objectlistIterator.hasNext()){
				...
				String isKindOfExperiment = (String)objectInfo.get(ProgramCentralConstants.SELECT_KINDOF_EXPERIMENT_PROJECT);
				...
				if("True".equalsIgnoreCase(isKindOfExperiment) || "True".equalsIgnoreCase(isKindOfProjectBaseline) || "Inactive".equalsIgnoreCase(currentState)){
      ...
      public boolean hasAccessToRemoveGovernedItemsAction(Context context, String[] args) throws MatrixException {
	    ...
	    final String DEFAULT_CONSTRAINT_TYPE = "attribute["+DomainConstants.ATTRIBUTE_DEFAULT_CONSTRAINT_TYPE+"]";
	    ...
	    busSelects.add(DEFAULT_CONSTRAINT_TYPE);
	    ...
	    busSelects.add(SELECT_TASK_CONSTRAINT_TYPE);
	    	...
	    	ContextUtil.pushContext(context);
	    	MapList objectList = palObj.getRelatedObjects(
	    			context,                // context
	    			Task.RELATIONSHIP_PROJECT_ACCESS_KEY+","+Task.RELATIONSHIP_PROJECT_ACCESS_LIST,    // relationshipPattern
	    			TYPE_PROJECT_MANAGEMENT,// typePattern
	    	...
	    	objectList.forEach(map->{
	    		...
	    		Dataobject taskObj 			= projectSeqData.get(taskPhysicalId);

	    		taskMap.put(SubtaskRelationship.SELECT_TASK_WBS,(String) taskObj.getDataelements().get(ProgramCentralConstants.KEY_WBS_ID));
	    		taskMap.put(SubtaskRelationship.SELECT_SEQUENCE_ORDER,(String) taskObj.getDataelements().get(ProgramCentralConstants.KEY_SEQ_ID));
	    	});

	    	objectList.sortStructure(SubtaskRelationship.SELECT_SEQUENCE_ORDER,  "ascending","integer");
	    ...
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Common.WBS", language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Common.Name", language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Common.Type", language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Common.EstimatedDuration", language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Common.EstimatedStartDate",language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Common.EstimatedFinishDate",language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.Button.ManageDependency",language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxFramework.Command.Assignee",language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.WBS.ConstraintType",language) + seperator);
	    headerLine.append(ProgramCentralUtil.i18nStringNow("emxProgramCentral.WBS.ConstraintDate",language));
	    ...
	    headerLine.append(seperator);
	    headerLine.append((String) projectMap.get(DEFAULT_CONSTRAINT_TYPE));
	    headerLine.append(seperator);
	    ...
	    taskSelects.add(SELECT_NAME);
	        ...
	        if (listPreds == null) {
	            preString = "";
	    			...
	    			Dataobject predecessor = projectSeqData.get((String) listPreds);
	              ...
	              if(((String)listLagTimes).startsWith("-")){
	    					preString += (String) listLagTimes + " " + listLagTimesDurationUnit;
	              } else {
	    					preString += "+" + (String) listLagTimes + " " + listLagTimesDurationUnit;
	              }
	            }
	            ...
	            StringList slag = (StringList) listLagTimes;
	    			StringList slagDurationUnit = (StringList) listLagTimesDurationUnit;
	            ...
	            for (int i =0; i<sl.size(); i++) {
	                if(i!=0){
	                  ...
	                  preString="\""+preString;
	    				...
	    				Dataobject predecessor = projectSeqData.get((String) sl.elementAt(i));
	    				String seqNo = (String) predecessor.getDataelements().get(ProgramCentralConstants.KEY_SEQ_ID);

	    				//String preId = (String)wbsIndexMap.get((String) sl.elementAt(i));
	    				preString = preString + seqNo + ":" + (String) st.elementAt(i);
	    				if (slag != null && !"".equals(slag) && (null != slagDurationUnit)){
	                  if (((String)slag.elementAt(i)).startsWith("-")){
	    						preString += (String) slag.elementAt(i) + " " + slagDurationUnit.elementAt(i);
	                  } else {
	    						preString += "+" + (String) slag.elementAt(i) + " " + slagDurationUnit.elementAt(i);
	                  }
	                }
	        ...
	        thisLine.append(thisTask.get(SELECT_TASK_CONSTRAINT_TYPE) + seperator);
	...
	public void updateProjectScheduleAttribute(Context context,String[]args)throws Exception
	{
		...
		project.setAttributeValue(context, 
				...
				newScheduleValue);
			DomainConstants.MULTI_VALUE_LIST.add(SELECT_TASK_ASSIGNEE);
    ...
    protected MapList getProjectList(Context context, String[] args, String Mode) throws MatrixException
    {
		...
		public boolean hasAccessToPMCCalculateFloatAndCriticalPathCommand(Context context,String args[]) throws Exception
		{
			...
			try {
				...
				if(ProgramCentralUtil.isNotNullString(selectedTable) && (excludeTableList.contains(selectedTable))){
					hasAccess = false;
				}
			} catch (Exception e) {
